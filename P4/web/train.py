...
class Net(nn.Module):
    def __init__(self):
        ...
        self.layers = nn.Sequential(
            ...
            nn.Conv2d(self.n_class, self.n_class, 1, padding=0),
            nn.ReLU(inplace=True),
            nn.Conv2d(self.n_class, self.n_class, 1, padding=0),
            nn.ReLU(inplace=True),
            nn.Conv2d(self.n_class, self.n_class, 1, padding=0),
...
def main():
    ...
    train_data = FacadeDataset(flag='train', data_range=(0,20), onehot=False, transform=transform)
    train_loader = DataLoader(train_data, batch_size=1)
    test_data = FacadeDataset(flag='test_dev', data_range=(0,114), onehot=False, transform=transform)
    test_loader = DataLoader(test_data, batch_size=1)
    ap_data = FacadeDataset(flag='test_dev', data_range=(0,114), onehot=True, transform=transform)
